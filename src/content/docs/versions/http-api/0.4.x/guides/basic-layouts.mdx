...
sidebar:
  order: 1
slug: http-api/0.4.x/http-api/guides/basic-layouts
---
...
<Steps>
  1. Init Smelter and connect inputs and outputs
     ...
     Start Smelter server, register inputs (`input_1`, `input_2`) and register video output (`output_1`) similarly to examples shown in [`Quick start`](/http-api/0.4.x/guides/quick-start) guide.
  ...
  5. Put one of the inputs in the corner
     ...
     * Root `View` has 2 child components, but only one is positioned statically.
       (See [absolute positioning](/http-api/0.4.x/fundamentals/concepts/layouts/#absolute-positioning))
