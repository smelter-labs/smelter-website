...
sidebar:
  order: 2
slug: http-api/0.4.x/http-api/guides/web-rendering
---
...
* **[`SMELTER_WEB_RENDERER_ENABLE`](/http-api/0.4.x/deployment/configuration#smelter_web_renderer_enable)** - For deployment, this environment variable must be set to `true` to enable the web renderer functionality.
...
<Steps>
  ...
  2. Create an output scene with a `WebView` component.
     ...
     The `WebView` component needs to use an `instance_id` that has already been registered.
