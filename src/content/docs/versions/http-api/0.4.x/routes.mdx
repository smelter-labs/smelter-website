...
API is served by default on the port 8081. Different port can be configured using [`SMELTER_API_PORT`](/http-api/0.4.x/deployment/configuration#smelter_api_port) environment variable.
...
* `instance_id` - ID that can be provided using [`SMELTER_INSTANCE_ID`](/http-api/0.4.x/deployment/configuration#smelter_instance_id) environment variable. Defaults to random number.
