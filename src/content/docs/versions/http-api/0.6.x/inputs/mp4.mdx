---
title: MP4
slug: http-api/0.6.x/http-api/inputs/mp4
---
...
Start playing from a specific timestamp in milliseconds. If `loop` is enabled, subsequent iterations start from the beginning. The input can also be seeked at runtime with the [`update input`](/http-api/0.6.x/routes#update-input) request.
