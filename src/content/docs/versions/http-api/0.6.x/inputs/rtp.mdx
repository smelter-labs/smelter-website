---
title: RTP
slug: http-api/0.6.x/http-api/inputs/rtp
---
...
range will be returned from [`register-input` request](/http-api/0.6.x/routes#register-input).
...
To find value for the `audio_specific_config` field search for the `config=<HEX_STRING>`. The `HEX_STRING` is the required config.
