...
sidebar:
  order: 0
slug: http-api/0.6.x/http-api/guides/quick-start
---
...
<Steps>
  ...
  2. Register an output stream and send start request.
     ...
     Sending [start request](/http-api/0.6.x/routes#start-request) will cause all registered outputs
     ...
     <CollapsibleDetails summaryTitle="Option 3: MP4 output">
       ...
       When you want to finish recording, send [unregister request](/http-api/0.6.x/routes#unregister-output):
  ...
  3. Register 2 input streams.
     ...
     logic would apply for any other [input type](/http-api/0.6.x/overview#inputs).
  ...
  5. Update output to show both of the input streams.
     ...
     [`Tiles`](/http-api/0.6.x/components/tiles) component is a simple way to display element in
