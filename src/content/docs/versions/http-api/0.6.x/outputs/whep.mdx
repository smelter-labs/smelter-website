---
title: WHEP server
slug: http-api/0.6.x/http-api/outputs/whep
---
...
To connect new output, register it first with [`register-output` request](/http-api/0.6.x/routes#register-output)
...
<Aside type="tip">
  You can play this output straight from your browser with the hosted
  [WHEP player](https://smelter-labs.github.io/tools/#whep-player?url=http://localhost:9000/whep/output_1\&token=example) —
  set `url` to `http://localhost:9000/whep/<output_id>` and `token` to the output's bearer token.
</Aside>
