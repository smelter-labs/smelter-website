...
sidebar:
  order: 1
slug: http-api/0.6.x/http-api/outputs/encoders/ffmpeg-h264
---
...
For latency-sensitive outputs you can reduce the delay introduced by the encoder itself with [`ffmpeg_options`](#ffmpeg_options):
...
FFmpeg picks whichever H.264 encoder is available in the build it was compiled with. Smelter passes a different set of default options depending on which encoder was picked. Anything you set in [`ffmpeg_options`](#ffmpeg_options) is applied on top of these defaults and overrides them.
