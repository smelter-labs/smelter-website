...
slug: http-api/0.6.x/http-api/outputs/whip
---
...
The `type: "any"` specifies that any audio encoder supported by Smelter can be used. If `"any"` is not included in
the `encoder_preferences` list, Smelter will only use the encoders explicitly listed in the preferences.
