...
sidebar:
  order: 0
slug: ts-sdk/0.4.x/ts-sdk/web-client/smelter
---
...
The returned `InputHandle` lets you control the input after registration. For [MP4](/ts-sdk/0.4.x/inputs/mp4) inputs it is an `Mp4InputHandle` that additionally supports seeking; for [WHIP server](/ts-sdk/0.4.x/inputs/whip) inputs it is a `WhipInputHandle` that exposes the negotiated endpoint URL and bearer token.
