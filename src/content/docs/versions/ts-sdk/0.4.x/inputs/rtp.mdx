---
title: RTP
slug: ts-sdk/0.4.x/ts-sdk/inputs/rtp
---
...
range will be returned from `Smelter.registerInput`.
...
To find value for the `audioSpecificConfig` field search for the `config=<HEX_STRING>`. The `HEX_STRING` is the required config.
