---
title: MP4
slug: ts-sdk/0.4.x/ts-sdk/inputs/mp4
---
...
Start playing from a specific timestamp in milliseconds. If `loop` is enabled, subsequent iterations start from the beginning. The input can also be seeked at runtime through the handle returned from [`registerInput`](/ts-sdk/0.4.x/nodejs/smelter#registerinput).
