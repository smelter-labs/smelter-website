---
title: Overview
slug: ts-sdk/0.4.x/ts-sdk/side-channel/overview
---
...
results back into the TypeScript app driving the composition.
...
page for the exact field definition (e.g. [`MP4`](/ts-sdk/0.4.x/inputs/mp4#sidechannel)).
...
See the [Python API reference](/ts-sdk/0.4.x/side-channel/python) for the full surface
(sync + async, audio batches, channel discovery).

## Guides

* [Side channel - speech-to-text](/ts-sdk/0.4.x/guides/side-channel-speech-to-text): Whisper sidecar that pushes recognised lines into a Zustand store so the JSX composition renders live subtitles.
* [Side channel - object detection](/ts-sdk/0.4.x/guides/side-channel-object-detection): YOLO sidecar that pushes tracked bounding boxes into a Zustand store so the JSX composition renders animated overlays.
