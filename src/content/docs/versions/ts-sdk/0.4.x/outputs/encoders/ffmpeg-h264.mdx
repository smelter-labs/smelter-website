...
sidebar:
  order: 1
slug: ts-sdk/0.4.x/ts-sdk/outputs/encoders/ffmpeg-h264
---
...
threads=0         # auto-detect thread count
g=...             # GOP size in frames, from "keyframeIntervalMs"
crf=23            # quality-based VBR, only if "bitrate" is not set
b=...             # average bitrate, only if "bitrate" is set
maxrate=...       # maximum bitrate, only if "bitrate" is set
bufsize=...       # equal to the average bitrate, only if "bitrate" is set
```
...
* **Default value**: Dependent on the encoder used:
  ...
  * **libopenh264**, **h264\_videotoolbox**: Calculated based on framerate and resolution. Eg. for 30 FPS in 1080p
