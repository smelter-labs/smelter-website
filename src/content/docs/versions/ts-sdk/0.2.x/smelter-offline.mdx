...
slug: ts-sdk/0.2.x/ts-sdk/smelter-offline
---
...
`OfflineSmelter` is intended for [offline processing](/ts-sdk/0.2.x/fundamentals/glossary#offline-processing) e.g. if you need to combine few mp4 files together.
It has simplified API compared to the [`Smelter`](/ts-sdk/0.2.x/smelter) class, optimized for rendering just one output file as a result.
...
* [`LocallySpawnedInstnaceManager`](/ts-sdk/0.2.x/managers/node-locally-spawned) (**Default Configuration**) - Automatically download the necessary Smelter binaries and launch the server on your local machine.
