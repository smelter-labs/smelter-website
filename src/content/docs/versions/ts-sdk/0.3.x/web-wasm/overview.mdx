...
sidebar:
  order: 0
slug: ts-sdk/0.3.x/ts-sdk/web-wasm/overview
---
...
When using this approach, your React code runs inside the client's browser, and any updates
to the React component tree are handled by the WASM module running in a separate Web Worker.
Since the engine is compiled to WebAssembly, it handles all media processing and output generation
locally, eliminating the need for a separate Smelter server.

Check out [`Smelter` documentation](/ts-sdk/0.3.x/web-wasm/smelter).
