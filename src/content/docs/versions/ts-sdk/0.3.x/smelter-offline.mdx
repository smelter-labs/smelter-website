...
slug: ts-sdk/0.3.x/ts-sdk/smelter-offline
---
...
It has simplified API compared to the [`Smelter`](/ts-sdk/0.3.x/smelter) class, optimized for rendering just one output file as a result.
...
* [`LocallySpawnedInstnaceManager`](/ts-sdk/0.3.x/managers/node-locally-spawned) (**Default Configuration**) - Automatically download the necessary Smelter binaries and launch the server on your local machine.
* [`ExistingInstanceManager`](/ts-sdk/0.3.x/managers/node-existing-instance) - Connect to a smelter server instance that is already running.
...
* `output` - Protocol and encoder options for the output. Visit output documentation to learn more:
  ...
  * [WHIP](/ts-sdk/0.3.x/outputs/whip)
