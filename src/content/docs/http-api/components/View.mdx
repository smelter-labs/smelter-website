...
Below is an example of how View can be used to in output stream. In this case, output is solid green window.

<CollapsibleDetails summaryTitle='Register RTP output with View component' open>
<Code code={`
...
`} lang='http' ins={{range: '13-16'}} />
</CollapsibleDetails>

After registering the output, you can start the video stream by sending a [start request](/http-api/routes#start-request). The stream will be available on port 8004.
