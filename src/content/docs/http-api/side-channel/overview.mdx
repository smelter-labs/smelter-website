...
for the exact field definition (e.g. [`MP4`](/http-api/inputs/mp4#side_channel)).
...
See the [Python API reference](/http-api/side-channel/python) for the full surface
(sync + async, audio batches, channel discovery).

## Guides

- [Side channel - speech-to-text](/http-api/guides/side-channel-speech-to-text): Whisper sidecar that posts live subtitles back to the composed output.
- [Side channel - object detection](/http-api/guides/side-channel-object-detection): YOLO sidecar that draws bounding boxes around tracked objects.
