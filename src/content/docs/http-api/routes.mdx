...
- `pause` - When set to `true` the input will stop delivering frames/samples; rendering keeps the last frame and audio mixing skips this input until resumed. Setting to `false` resumes the input. Only supported for [MP4](/http-api/inputs/mp4) inputs.
- `seek_ms` - Seek to a specific position in milliseconds. Only supported for [MP4](/http-api/inputs/mp4) inputs.

Pausing and resuming emit [`VIDEO_INPUT_PAUSED`/`AUDIO_INPUT_PAUSED`](/http-api/events#video_input_paused) and a new [`VIDEO_INPUT_PLAYING`/`AUDIO_INPUT_PLAYING`](/http-api/events#video_input_playing) WebSocket events.
...
Register an image asset. Request body is defined in the [image](/http-api/resources/image) docs.
...
Register a shader. Request body is defined in the [shader](/http-api/resources/shader) docs.
...
Register a web renderer instance. Request body is defined in the [web renderer](/http-api/resources/web-renderer) docs.
...
start using new fonts with a [`Text`](/http-api/components/text) component.
...
Establish WebSocket connection to listen for Smelter events. List of supported events and their descriptions can be found [here](/http-api/events).
