...
```tsx
type EncoderBitrate = { average_bitrate: number; max_bitrate: number; } | number;
```
</CollapsibleDetails>
...
```tsx
type EncoderPreset = "quality" | "balanced" | "fast" | "low_latency_quality" | "low_latency_balanced" | "low_latency_fast";
```
</CollapsibleDetails>
...
- **Supported values**:
  ...
  - `quality_low_latency` - prioritizing quality, tuned for low latency streaming
  - `balanced_low_latency` - compromise between speed and quality, tuned for low latency streaming
  - `fast_low_latency ` - prioritizing speed, tuned for low latency streaming
