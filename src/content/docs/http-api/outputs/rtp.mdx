...
type VideoOptions = {
  ...
  encoder: {
    type: "ffmpeg_h264" | "ffmpeg_vp8" | "ffmpeg_vp8" | "vulkan_h264";
...
- **Supported values**:
  ...
  - `ffmpeg_vp8`
  - `ffmpeg_vp9`
  - `vulkan_h264` - Hardware encoder. Requires GPU that supports Vulkan Video encoding.

##### `options`
  - **Type**: [`VideoEncoderOptions`](/http-api/options/videoencoderoptions)
