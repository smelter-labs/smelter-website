...
<Aside>
If required FFmpeg version is not detected, appropriate `libav*.so`/`libav*.dylib` dependencies will be
downloaded on startup (except for macOS Intel version). However, you will need to make sure that dependencies
of libav libraries are installed. In most cases it will work if you install any FFmpeg version.
</Aside>
...
```sh frame=none
cargo build -r --no-defatult-features
```
