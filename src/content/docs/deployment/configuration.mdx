...
Specify port where MoQ server should be served.

- **Defaults to**: `4443`

### `SMELTER_START_MOQ_SERVER`

If enabled, the Smelter server will start MoQ server. If disabled it won't be possible to register MoQ inputs.

- **Defaults to**: `true`

### `SMELTER_MOQ_TLS_CERT_FILE`

Path to a TLS certificate file for the MoQ server. Both `SMELTER_MOQ_TLS_CERT_FILE` and `SMELTER_MOQ_TLS_KEY_FILE`
are required for the MoQ server to start.

### `SMELTER_MOQ_TLS_KEY_FILE`

Path to a TLS private key file for the MoQ server. Both `SMELTER_MOQ_TLS_CERT_FILE` and `SMELTER_MOQ_TLS_KEY_FILE`
are required for the MoQ server to start.
