...
<Aside type="caution">`TEXTURE_BINDING_ARRAY` and `IMMEDIATES` are always required and cannot be overwritten using this environment variable.</Aside>

### `SMELTER_RENDER_MAX_LAYOUTS_COUNT`
Maximum number of layouts that can be rendered as part of a single group of directly nested layout components (e.g. `View`, `Tiles`, `Rescaler`). If the limit is exceeded, some elements will not be rendered.
- **Defaults to**: `100`
- **Supported values**:
...
### `SMELTER_RTMP_SERVER_PORT`
Specify port where RTMP server should be served.

- **Defaults to**: `1935`

### `SMELTER_START_RTMP_SERVER`

If enabled, the Smelter server will start RTMP server. If disabled it won't be possible to register RTMP inputs.

- **Defaults to**: `true`

### `SMELTER_RTMP_TLS_CERT_FILE`

Path to a TLS certificate file for the RTMP server. When both `SMELTER_RTMP_TLS_CERT_FILE`
and `SMELTER_RTMP_TLS_KEY_FILE` are set, the RTMP server will accept RTMPS connections.

### `SMELTER_RTMP_TLS_KEY_FILE`

Path to a TLS private key file for the RTMP server. When both `SMELTER_RTMP_TLS_CERT_FILE`
and `SMELTER_RTMP_TLS_KEY_FILE` are set, the RTMP server will accept RTMPS connections.

### `SMELTER_MOQ_SERVER_PORT`
Specify port where the MoQ server should be served.

- **Defaults to**: `4443`

### `SMELTER_START_MOQ_SERVER`

If enabled, the Smelter server will start the MoQ server. If disabled it won't be possible to register MoQ inputs.

- **Defaults to**: `true` when both `SMELTER_MOQ_TLS_CERT_FILE` and `SMELTER_MOQ_TLS_KEY_FILE` are set, otherwise `false`

### `SMELTER_MOQ_TLS_CERT_FILE`

Path to a TLS certificate file for the MoQ server. When both `SMELTER_MOQ_TLS_CERT_FILE`
and `SMELTER_MOQ_TLS_KEY_FILE` are set, the MoQ server uses that certificate. Otherwise a
self-signed certificate is auto-generated at `~/.smelter/moq_cert.pem` / `~/.smelter/moq_key.pem`
(14-day validity). Development clients must be configured to trust it.

### `SMELTER_MOQ_TLS_KEY_FILE`

Path to a TLS private key file for the MoQ server. When both `SMELTER_MOQ_TLS_CERT_FILE`
and `SMELTER_MOQ_TLS_KEY_FILE` are set, the MoQ server uses that certificate. Otherwise a
self-signed certificate is auto-generated at `~/.smelter/moq_cert.pem` / `~/.smelter/moq_key.pem`
(14-day validity). Development clients must be configured to trust it.

### `SMELTER_MOQ_DISABLE_TLS_VERIFICATION`

If enabled, all MoQ client inputs and outputs skip TLS server certificate verification when connecting to a relay.

<Aside type="caution">This option makes the connection vulnerable to man-in-the-middle attacks. Do not use in production.</Aside>
...
Port range in format `"START:END"`. Defines what ports will be used for WebRTC connections used by all WHIP/WHEP input/outputs.

This option can't be used together with `SMELTER_WEBRTC_UDP_MUX_PORT`

### `SMELTER_WEBRTC_UDP_MUX_PORT`

An UDP port number, when provided WebRTC traffic from all WHIP/WHEP inputs/outputs will be muxed on a single port.

This option can't be used together with `SMELTER_WEBRTC_UDP_PORT_RANGE`

### `SMELTER_WEBRTC_1_TO_1_NAT_IPS`

List of comma separated IP addresses. ICE candidates generated in the process of WebRTC connection negotiation will use those
IPs instead of IP of the host.

### `SMELTER_SIDE_CHANNEL_SOCKET_DIR`

Directory where Smelter creates [side channel](/http-api/side-channel/overview) Unix sockets for inputs registered with `side_channel` enabled. The directory must be empty at startup and is created if missing.

- **Defaults to**: a unique subdirectory of `$XDG_RUNTIME_DIR` (or `/tmp` on macOS, `env::temp_dir()` elsewhere) named `smelter_side_channel_<random number>`.
