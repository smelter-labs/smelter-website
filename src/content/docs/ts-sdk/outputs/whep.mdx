...
An output type that allows streaming video and audio from Smelter over WHEP.
...
```tsx title=whepOutputExample.tsx
...
async function run() {
  ...
  console.log(`WHEP stream available at: http://localhost:9000${endpointRoute}`);
}
void run();
```
