...
There are different variants of the SDK that allow you to run Smelter in different environments.
You need to decide where to run React code that defines video layout and where to execute
video processing.

- ##### Node.js
  NPM package: `@swmansion/smelter-node`

  React code executes as part of the Node.js process, generates scene updates based on React state
  changes, and sends them to Smelter server as HTTP requests. By default, Smelter server is 
  automatically started on the same machine as Node.js process. However, you can always
  deploy it yourself and point the library 

- ##### Browser (Client)
  NPM package: `@swmansion/smelter-web-client`

  React code is executed inside the browser as part of the website code. If you are using React for your
  frontend it is a separate for React running inside Smelter. SDK generates scene updates
  based on React state changes, and sends them to Smelter server as HTTP requests. 

- ##### Browser (WASM)
  
  NPM package: `@swmansion/smelter-web-wasm`

  Both React code and Smelter code is executed inside the browser as part of the website code. Smelter code
  is compiled to WASM, all the GPU based logic is converted to WebGL and runs in a separate JavaScript
