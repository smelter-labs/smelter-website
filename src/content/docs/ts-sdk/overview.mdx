...
Visit [Smelter class documentation](/ts-sdk/smelter) for more details
...
### Inputs
You can handle most of the stream composing by using React components. However, delivering multimedia to the smelter requires registering the input beforehand. You can register an input by using [Smelter.registerInput](/ts-sdk/smelter#registerinput), which accepts `inputId` and `inputType`. Right now, we support two input types:
