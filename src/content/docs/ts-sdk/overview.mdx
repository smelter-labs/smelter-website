...
### LiveCompositor
The primary element required to start using the Live Compositor TypeScript SDK is the `LiveCompositor` class. This class is crucial for initializing and managing the Live Compositor server instance. You can import `LiveCompositor` class from:    
    - `@live-compositor/node`
    - `@live-compositor/node`
    - `@live-compositor/node`

Visit [LiveCompositor class documentation](/ts-sdk/live-compositor) for more details
...
- [InputStream](/ts-sdk/components/input-stream)
...
- [WebView](/ts-sdk/components/web-view)
...
### Inputs
You can handle most of the stream composing by using React components. However, delivering multimedia to the compositor requires registering the input beforehand. You can register an input by using [LiveCompositor.registerInput](/ts-sdk/live-compositor#registerinput), which accepts `inputId` and `inputType`. Right now, we support two input types:
