...
## new Smelter()
Creates new a smelter configuration. You have to call init first before this object can be used.
...
- **Type**: [`SmelterManager`](/ts-sdk/smelter#new-smelter)
<br/>
...
- **Existing Server Configuration:** Choose `ExistingInstance` from `@swmansion/smelter-node` if you need to connect to a smelter instance that is already running.
