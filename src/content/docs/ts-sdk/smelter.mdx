...
The returned `InputHandle` lets you control the input after registration. For [MP4](/ts-sdk/inputs/mp4) inputs it is an `Mp4InputHandle` that additionally supports seeking; for [WHIP](/ts-sdk/inputs/whip) inputs it is a `WhipInputHandle` that exposes the negotiated endpoint URL and bearer token.
...
<Aside>`pause()` / `resume()` are only supported for [MP4](/ts-sdk/inputs/mp4) inputs. Calling them on other input types will throw.</Aside>
