...
### seekMs
Start playing from a specific timestamp in milliseconds. If `loop` is enabled, subsequent iterations start from the beginning. The input can also be seeked at runtime through the handle returned from [`registerInput`](/ts-sdk/smelter#registerinput).
