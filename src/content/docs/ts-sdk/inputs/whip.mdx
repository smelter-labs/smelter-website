...
<Aside>
If available from the source, the audio track is **always** used.

In the following example, both the audio and video tracks will be included if the source provides them:
```tsx
await smelter.registerInput("example", {
  type: "whip",
});
